...
void *client_thread_func (void *arg)
{
    ...
    for (i = 0; i < num_peers; i++) {
        for (j = 0; j < num_concurr_msgs; j++) {
            ret = post_send (msg_size, lkey, (uint64_t)buf_ptr, 
                            (uint32_t)config_info.rank * num_peers + i, qp[i], buf_ptr);
    ...
    while (stop != true) {
        ...
        {
            if (wc[i].status != IBV_WC_SUCCESS) {
                check (wc[i].opcode == IBV_WC_SEND, "thread[%ld]: recv failed status: %s; wr_id = %"PRIx64"",
	        ...
	        if (wc[i].opcode == IBV_WC_RECV) {
                ...
                if (imm_data == MSG_CTL_STOP) {
                    ...
                    post_send (msg_size, lkey, 0, imm_data, qp[imm_data % num_peers], msg_ptr);
                }
                /* post a new receive */
