...
void *server_thread (void *arg)
{
    ...
    while (stop != true || n != 0) {
        ...
        for (i = 0; i < n; i++) 
        {
            ...
            if (wc[i].opcode == IBV_WC_SEND) {
                ...
                debug ("imm_data = %ld, wr_id = %ld", imm_data, wc[i].wr_id);
                if (wc[i].wr_id) {
                    ret = post_send (msg_size, lkey, wc[i].wr_id, 
                        imm_data, qp[(qp_offset++) % num_peers], msg_ptr);
                }
            }
    ...
    // for (i = 0; i < num_peers; i++) {
    //     j = 0;
    //     while((ret = post_send (0, lkey, IB_WR_ID_STOP, 
    //     MSG_CTL_STOP, qp[i], ib_res_array[thread_id].ib_buf)))
    //         if (j++ >= 10) break;
    //     check (ret == 0, "thread[%ld]: qp[%d] failed to signal the client to stop", thread_id, i);
    // }
